...
func (p *TextParser) Parse(r io.Reader, pkgName string) (*Report, error) {
	...
	for {
		...
		if strings.HasPrefix(line, "=== RUN ") {
			...
			benchmarks = append(benchmarks, &Benchmark{
				Name:     matches[1],
				Duration: p.parseNanoseconds(matches[3]),
				Bytes:    bytes,
				Allocs:   allocs,
			})
			...
			} else if matches[1] == "FAIL" && !p.containsFailures(tests) && len(buffers[cur]) > 0 {
			...
			report.Packages = append(report.Packages, Package{
				Name:        matches[2],
				Duration:    p.parseSeconds(matches[3]),
				Tests:       tests,
				Benchmarks:  benchmarks,
				CoveragePct: coveragePct,

				Time: int(p.parseSeconds(matches[3]) / time.Millisecond), // deprecated
			})
			...
			test.Name = matches[2]
			test.Duration = p.parseSeconds(matches[3])
...
func (p *TextParser) parseSeconds(t string) time.Duration {
	if t == "" {
...
func (p *TextParser) parseNanoseconds(t string) time.Duration {
...
func (p *TextParser) findTest(tests []*Test, name string) *Test {
...
func (p *TextParser) containsFailures(tests []*Test) bool {
